...
def main():
    ...
    if args.gpu >= 0:
        ...
        caffe.set_mode_cpu()
        print('Using CPU')
    pretrained = '/space/export/data/zwie_th/DLR/NeuralNetworks/dilation/pretrained/dilated_convolution_context_coco.caffemodel'
    ...
    if args.context:
        ...
        model_path = join(dirname(__file__), 'models',
                          'dilated_convolution_context_org.prototxt')
    else:
        suffix = '_front.png'
        model_path = join(dirname(__file__), 'models',
                          'dilated_convolution_front_org.prototxt')
