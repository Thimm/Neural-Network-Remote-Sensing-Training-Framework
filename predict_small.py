...
def predict(model_path, pretrained, input_path, output_path):
    ...
    np.save('nparray.ar', zoom_prob.transpose([1, 2, 0]), True)
...
def main():
    ...
    if args.gpu >= 0:
        ...
        caffe.set_mode_cpu()
        print('Using CPU')
    pretrained = join(dirname(__file__), '/space/export/data/zwie_th/DLR/EichenauTraining/Snapshots/Dilation/20160421/12_iter_400.caffemodel')
    ...
    if args.context:
        ...
        model_path = join(dirname(__file__), 'models',
                          'dilated_convolution_context_test.prototxt')
    else:
        suffix = '_front.png'
        model_path = join(dirname(__file__), 'models',
                          'dilated_convolution_front_test.prototxt')
